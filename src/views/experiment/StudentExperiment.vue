...
const data = reactive({
  ...
  subcourseListLoading: true,
  notificationListLoading: true,
  studyProgressListLoading: true,
...
const getCourseDetail = async (courseId:number, page:number, count:number) => {
  ...
  data.subcourseListLoading = false;
}
// 分页
...
const searchNotification = async (classId: number) => {
  ...
  const res = await getNotificationList(classId, data.inputNotification, data.notificationPage, data.notificationCount);
  data.currentClassId = classId;
  data.notificationList = res.data.list;
  data.notificationTotal = res.data.total;
  data.notificationListLoading = false;
  data.searchNotificationLoading = false;
}
// 分页
const handleNotificationSizeChange = (val: any) => {
  searchNotification(data.currentClassId);
}
const handleNotificationCurrentChange = (val: any) => {
  data.notificationPage = val;
  searchNotification(data.currentClassId);
}
...
// 搜索学习进度
const searchStudyProgress = async () => {
  const res = await getStuStudyProgessDetail(data.currentStudyProgressId, data.currentExperimentId, data.studyProgressPage, data.studyProgressCount);
  data.studyProgressList = res.data.list;
  data.studyProgressTotal = res.data.total;
}
// 打开学习进度框
const openStudyProgressModal = async (id: number) => {
  data.currentStudyProgressId = id;
  ...
  data.studyProgressListLoading = false;
}
// 学习进度分页
...
<template>
  <div class="experiment-page">
    ...
    <div class="experiment-list">
      <el-row class="experiment-list-container" :gutter="12" v-loading="data.courseListLoading">
        <el-empty v-if="data.courseList.length === 0 && !data.courseListLoading" description="暂无课程实验信息"/>
          ...
          <el-col :span="8" v-for="course in data.courseList" :key="course.id">
            <el-card :body-style="{ padding: '10px', margin: '5px' }">
              ...
              <div class="course-card-btn">
                <el-button type='primary'>{{ course.total_time }}课时</el-button>
  ...
  <el-dialog v-model="data.experimentDetailModalVisible" title="实验详情" width="1200" center>
    <!-- 章节列表 -->
    <div class="experiment-dialog" v-loading="data.subcourseListLoading">
      <el-empty v-if="data.subcourseList.length === 0 && !data.subcourseListLoading" description="暂无章节信息" />
  ...
  <el-dialog v-model="data.classNotificationModalVisible" title="班级通知" width="1200" center>
    <div class="experiment-dialog">
      ...
      <!-- 通知列表 -->
      <div class="notification-list" v-loading="data.notificationListLoading">
        <el-empty v-if="data.notificationList.length === 0 && !data.notificationListLoading" description="暂无通知" />
        <el-table v-if="data.notificationList.length !== 0" :data="data.notificationList" border style="width: 100%" max-height="400">
        ...
        <!-- 分页 -->
        <el-pagination
          v-if="data.notificationList.length !== 0"
          background 
          layout="prev, pager, next"
          :total="data.notificationTotal" 
          :page-size="data.notificationCount"
  ...
  <el-dialog v-model="data.studyProgressModalVisible" title="学习进度" width="1200" center>
    <!-- 学习进度列表 -->
    <div class="notification-list" v-loading="data.studyProgressListLoading">
      <el-empty v-if="data.studyProgressList.length === 0 && !data.studyProgressListLoading" description="暂无学习进度情况" />
      <el-table v-if="data.studyProgressList.length !== 0" :data="data.studyProgressList" border style="width: 100%">
